...
def register_user(request):
    if request.method == 'POST':
        ...
        if user:  # User exists, attempt to log in
            user = authenticate(username=username, password=password)
            if user is not None:
                login(request, user)  # Log the user in
                return Response({"message": f"User {username} logged in.", "user_id": user.id}, status=status.HTTP_200_OK)
            else:
                return Response({"error": "Invalid credentials."}, status=status.HTTP_400_BAD_REQUEST)
        else:  # User does not exist, proceed to register
            ...
            return Response({"message": f"User {username} registered as {role}.", "user_id": user.id}, status=status.HTTP_201_CREATED)
